...
SC_AGENT_IMPLEMENTATION(NonAtomicActionInterpreterAgent)
{
  ...
  try
  {
    ScAddr nonAtomicActionTemplateAddr = utils::IteratorUtils::getFirstByOutRelation(
          &m_memoryCtx,
          actionAddr,
          CoreKeynodes::rrel_1);
    ...
    ScAddr argumentsSet = utils::IteratorUtils::getFirstByOutRelation(
          &m_memoryCtx,
          actionAddr,
          CoreKeynodes::rrel_2);
...
bool NonAtomicActionInterpreterAgent::checkActionClass(ScAddr const & actionAddr)
{
  return m_memoryCtx.HelperCheckEdge(
        Keynodes::action_interpret_non_atomic_action,
        actionAddr,
...
{
  ScAddr templateKeyElement =  utils::IteratorUtils::getFirstByOutRelation(
        &m_memoryCtx,
        templateAddr,
      ...
      ScTemplateParams & templateParams)
{
  ...
  templateParams.Add(
        m_memoryCtx.HelperGetSystemIdtf(templateKeyElement),
        keyElementReplacement);
