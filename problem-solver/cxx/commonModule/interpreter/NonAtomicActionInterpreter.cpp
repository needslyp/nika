...
{
  ScAddr decompositionTuple = utils::IteratorUtils::getFirstByInRelation(
        context,
        nonAtomicActionAddr,
...
{
  ScAddr firstAction = utils::IteratorUtils::getFirstByOutRelation(
        context,
        decompositionTuple,
  ...
  if (!firstAction.IsValid())
  {
    throw std::runtime_error(
          "Non atomic action structure is incorrect. Failed to find first action.");
  }
  return firstAction;
}
