...
private:
  ...
  const std::vector<ScAddr> NODES_TO_REMOVE = { MessageKeynodes::concept_message,
                                                MessageKeynodes::rrel_entity,
                                                scAgentsCommon::CoreKeynodes::lang_ru,
                                                scAgentsCommon::CoreKeynodes::nrel_main_idtf };
  const std::vector<ScAddr> NODES_TO_REMOVE_BY_CONCEPT = { MessageKeynodes::concept_message,
                                                           scAgentsCommon::CoreKeynodes::lang_ru };
  ...
  std::vector<ScTemplateParams> findParametersList(
        const ScAddr & templateNode,
        const ScAddr & parametersNode);
  ...
  ScAddrVector getIncidentElements(
        const ScAddr & node,
        const ScAddr & structNode);
};
}
