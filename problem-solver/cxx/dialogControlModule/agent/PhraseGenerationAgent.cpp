...
SC_AGENT_IMPLEMENTATION(PhraseGenerationAgent)
{
  ...
  ScAddr phraseLink = IteratorUtils::getAnyByOutRelation(
        &m_memoryCtx,
        actionNode,
  ...
  ScAddr templateNode = IteratorUtils::getAnyByOutRelation(
        &m_memoryCtx,
        phraseLink,
        CoreKeynodes::nrel_phrase_template);
  ScAddr parametersNode = IteratorUtils::getAnyByOutRelation(
        &m_memoryCtx,
        actionNode,
...
bool PhraseGenerationAgent::checkActionClass(const ScAddr & actionNode)
{
  return m_memoryCtx.HelperCheckEdge(
        MessageKeynodes::action_phrase_generation,
        actionNode,
...
void PhraseGenerationAgent::generateSemanticEquivalent(const ScAddr & replyMessageNode, const ScAddr & templateNode)
{
  ...
  ScIterator3Ptr semanticEquivalentIterator = m_memoryCtx.Iterator3(
          MessageKeynodes::answer_structure,
          ScType::EdgeAccessConstPosPerm,
          ScType::Unknown
  );
...
string PhraseGenerationAgent::generatePhraseAnswer(const ScTemplateSearchResult & phraseSemanticResult,
                                                   ...
                                                   const std::string & text)
{
  ...
  for (auto & variable: variables)
  {
    ...
    string variableRegular = regex_replace(
          PhraseGenerationAgent::VAR_REGULAR,
          regex(PhraseGenerationAgent::VAR_CONST),
          variable);
...
)
{
  ...
  templateNode.Triple(structNode,
                      ScType::EdgeAccessVarPosPerm,
                      ScType::Unknown >> "_node");
  templateNode.Triple(conceptNode,
                      ScType::EdgeAccessVarPosPerm,
                      "_node");
...
ScAddrVector PhraseGenerationAgent::getIncidentElements(
      const ScAddr & node,
      const ScAddr & structNode)
{
  ...
  templateNode.TripleWithRelation(
        node,
        ScType::EdgeDCommonVar >> "_remove_arc_1",
        ScType::Unknown,
        ScType::EdgeAccessVarPosPerm,
        structNode
  );
  ...
  templateNode.TripleWithRelation(
        node,
        ScType::EdgeAccessVarPosPerm >> "_remove_arc_1",
        ScType::Unknown,
        ScType::EdgeAccessVarPosPerm,
        structNode
  );
