...
ScAddr PhraseSearcher::getNextPhraseClass(const ScAddr & phraseClassNode)
{
  const std::string VAR_TUPLE = "_tuple",
        VAR_EDGE_1 = "_edge_1",
        VAR_EDGE_2 = "_edge_2",
        VAR_D_COMMON_EDGE = "_d_common_edge",
        VAR_PHRASE_CLASS = "_phrase_class";

  ScTemplate templ;
  templ.Triple(
        ScType::NodeVarTuple >> VAR_TUPLE,
        ScType::EdgeAccessVarPosPerm >> VAR_EDGE_1,
        phraseClassNode);
  templ.Triple(
        VAR_EDGE_1,
        ScType::EdgeDCommonVar >> VAR_D_COMMON_EDGE,
        ScType::EdgeAccessVarPosPerm >> VAR_EDGE_2);
  templ.Triple(scAgentsCommon::CoreKeynodes::nrel_basic_sequence,
               ScType::EdgeAccessVarPosPerm,
               VAR_D_COMMON_EDGE);
  templ.Triple(
        VAR_TUPLE,
